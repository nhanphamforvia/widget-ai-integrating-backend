...
const executeCheckConsistency = async ({ visitedMap, checkQueue, prompt, role }) => {
  ...
  const promiseHandler = async ({ current, others, otherIds, currentId }) => {
    console.table({ current, others, otherIds, currentId });
...
const useChatCompletionForConsistency = async (similarTextGroups, prompt, role) => {
...
const useChatCompletionForIndividualItem = async (artifacts, prompt, role, batchSize = REQ_PER_TIME) => {
