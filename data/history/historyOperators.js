...
const sortSessionsByFields = (sessions, sort) => {
    ...
    sortFields.forEach(({ field, value }) => {
        switch (field) {
            ...
            case "createdAt": {
                sessions.sort((sessionA, sessionB) => value > 0 ? moment(sessionA.createdAt).diff(moment(sessionB.createdAt)) : moment(sessionB.createdAt).diff(moment(sessionA.createdAt)));
                break;
            }
...
const filterSessionsDateSpan = (sessions, type, dateStr) => {
    ...
    return sessions.filter(session => {  
        ...
        if (type === "fromDate")
            return moment(targetDate).startOf('day').isSameOrBefore(sessionDate);
        else 
            return moment(targetDate).endOf('day').isSameOrAfter(sessionDate);
    });
}
